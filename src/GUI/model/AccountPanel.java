...
public class AccountPanel extends JPanel implements FeatureControllerInterface {
    ...
    private void loadDataToTable(ArrayList<AccountDTO> list) {
        ...
        if (list != null) {
            for (AccountDTO acc : list) {
                String statusStr = (acc.getStatus() != null && acc.getStatus().equals("active")) ? "Hoạt động" : "Bị khóa";
                ...
                if (group != null) roleName = group.getGroupName();
    ...
    public void onDelete() {
        ...
        int confirm = JOptionPane.showConfirmDialog(this, "Chắc chắn muốn xóa tài khoản này?", "Cảnh báo", JOptionPane.YES_NO_OPTION);
        if (confirm == JOptionPane.YES_OPTION) {
            int accountId = (int) table.getValueAt(row, 0);
            
            // Xóa thật (Tạm thời để tham số người đang đăng nhập là -1)
            String msg = accountBUS.deleteAccount(accountId, -1); 
    ...
    public void onDetail() {
        int row = table.getSelectedRow();
        if(row == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn tài khoản cần xem chi tiết!", "Cảnh báo", JOptionPane.WARNING_MESSAGE);
            return;
        }
   ...
   @Override
    public void onExportExcel() {
        try {
            ...
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Lỗi khi xuất file Excel: " + e.getMessage(), "Lỗi Nghiêm Trọng", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    public void onImportExcel() {
        try {
            ...
            int confirm = JOptionPane.showConfirmDialog(this, msg, "Hướng dẫn Nhập Excel", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE);
            ...
            if (userSelection == JFileChooser.APPROVE_OPTION) {
                ...
                for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                    ...
                    try {
                        ...
                        boolean isAdded = accountBUS.addAccount(newAcc);
                ...
                JOptionPane.showMessageDialog(this, "Nhập file Excel hoàn tất!\n- Thành công: " + successCount + " tài khoản\n- Lỗi / Bỏ qua: " + errorCount + " dòng", "Kết quả Import", JOptionPane.INFORMATION_MESSAGE);
            }
        } catch (Exception e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Lỗi đọc file Excel! Vui lòng đảm bảo file không bị hỏng và đúng định dạng (.xlsx).", "Lỗi", JOptionPane.ERROR_MESSAGE);
        }
    }
