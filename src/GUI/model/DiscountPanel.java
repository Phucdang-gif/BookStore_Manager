...
public class DiscountPanel extends JPanel implements FeatureControllerInterface {
    ...
    private void initUI() {
        ...
        String[] columns = {"Mã KM", "Tên Chương Trình", "Giá Trị", "Đơn Tối Thiểu", "Giảm Tối Đa", "Ngày Bắt Đầu", "Ngày Kết Thúc", "Trạng Thái"};
    ...
    public void onEdit() {
        ...
        if(row == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn Chương trình khuyến mãi cần sửa!", "Cảnh báo", JOptionPane.WARNING_MESSAGE);
            return;
        }
    ...
    public void onDelete() {
        int row = table.getSelectedRow();
        if(row == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn Chương trình cần dừng!", "Cảnh báo", JOptionPane.WARNING_MESSAGE);
            return;
        }
        
        int confirm = JOptionPane.showConfirmDialog(this, "Bạn có chắc chắn muốn TẠM DỪNG chương trình khuyến mãi này?", "Xác nhận", JOptionPane.YES_NO_OPTION);
        if (confirm == JOptionPane.YES_OPTION) {
            ...
            if (isSuccess) {
                ...
                JOptionPane.showMessageDialog(this, "Lỗi: Không thể thực hiện thao tác này!", "Lỗi", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
